...
void Polygon::calculateSubPolygons()
{
    ...
    float xHi = xLo + std::ceil((aabb.upperBound.x - aabb.lowerBound.x) / step) * step;
    float yLo = aabb.lowerBound.y;
    float yHi = yLo + std::ceil((aabb.upperBound.y - aabb.lowerBound.y) / step) * step;
    
    for (float x = xLo; x <= xHi; x += step) {
        for (float y = yLo; y <= yHi; y += step) {
            std::vector<b2Vec2> points;
            points.emplace_back(x, y);
            points.emplace_back(x + step, y);
            points.emplace_back(x + step, y + step);
            points.emplace_back(x, y + step);
...
//                                           return jcv_clip_polygon_test_point(&boundsClipper, pt);
    ...
    for (int i = 0; i < diagram.numsites; i++) {
        ...
        if (new_points.size() >= 3) {
            Polygon newPolygon{new_points, material.type, isDynamic, false};
...
std::vector<Polygon> Polygon::splitIntoSubpolygons()
{
    ...
    if (points.size() < 3) {
        ...
        for (auto subPolygon : polys) {
            ...
            Polygon subPolygonResult = Polygon{subPolygonPoints, material.type, isDynamic, false};
...
std::vector<Polygon> Polygon::triangulate()
{
    ...
    for (auto triangle : cdt.triangles) {
        ...
        Polygon p{polygon, material.type, isDynamic, false};
        result.push_back(p);
    }
    return result;
}
